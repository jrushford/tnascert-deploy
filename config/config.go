...
func envLookup(key string, key_name string, re *regexp.Regexp) (string, error) {
	var val = ""
	r := re.FindAllStringSubmatch(key, -1)
	for _, v := range r {
		for _, vv := range v[1:] {
			if vv == "" {
				continue
				...
				s := os.Getenv(str)
				if s == "" {
					err := fmt.Errorf("no environment variable, %s, was found for the key %s", s, key_name)
					return val, err
				} else {
					val += os.Getenv(str)
				}
...
func (c *Config) checkConfig() error {
	...
	// lookup the ApiKey
	val, err := envLookup(c.ApiKey, "api_key", re)
	if err != nil {
		return err
	}
	if val != "" {
	...
	// lookup the Username
	val, err = envLookup(c.Username, "username", re)
	if err != nil {
		return err
	}
	if val != "" {
	...
	// lookup the Password
	val, err = envLookup(c.Password, "password", re)
	if err != nil {
		return err
	}
	if val != "" {
	...
	val, err = envLookup(c.CertBasename, "cert_basename", re)
	if err != nil {
		return err
	}
	if val != "" {
	...
	val, err = envLookup(c.ConnectHost, "connect_host", re)
	if err != nil {
		return err
	}
	if val != "" {
	...
	val, err = envLookup(c.ClientApi, "client_api", re)
	if err != nil {
		return err
	}
	if val != "" {
	...
	val, err = envLookup(c.DeleteOldCertsStr, "delete_old_certs", re)
	if err != nil {
		return err
	}
	if val == "" {
	...
	val, err = envLookup(c.FullChainPath, "full_chain_path", re)
	if err != nil {
		return err
	}
	if val != "" {
	...
	val, err = envLookup(c.PrivateKeyPath, "private_key_path", re)
	if err != nil {
		return err
	}
	if val != "" {
	...
	// lookup the port
	val, err = envLookup(c.PortStr, "port", re)
	if err != nil {
		return err
	}
	if val == "" && c.PortStr != "" {
	...
	// lookup the protocol
	val, err = envLookup(c.Protocol, "protocol", re)
	if err != nil {
		return err
	}
	if val != "" {
	...
	val, err = envLookup(c.TlsSkipVerifyStr, "tls_skip_verify", re)
	if err != nil {
		return err
	}
	if val == "" {
	...
	val, err = envLookup(c.AddAsUiCertificateStr, "add_as_ui_certificate", re)
	if err != nil {
		return err
	}
	if val == "" {
	...
	val, err = envLookup(c.AddAsFTPCertificateStr, "add_as_ftp_certificate", re)
	if err != nil {
		return err
	}
	if val == "" {
	...
	val, err = envLookup(c.AddAsAppCertificateStr, "add_as_app_certificate", re)
	if err != nil {
		return err
	}
	if val == "" {
	...
	// lookup the app_list
	val, err = envLookup(c.AppList, "app_list", re)
	if err != nil {
		return err
	}
	if val != "" {
	...
	val, err = envLookup(c.TimeoutSecondsStr, "timeoutSeconds", re)
	if err != nil {
		return err
	}
	if val == "" && c.TimeoutSecondsStr != "" {
	...
	val, err = envLookup(c.DebugStr, "timeout_seconds", re)
	if err != nil {
		return err
	}
	if val == "" {
